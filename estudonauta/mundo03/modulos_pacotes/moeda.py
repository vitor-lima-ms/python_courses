...
def resumo(valor, aumento=5, reducao=10):
    ...
    print('-' * (len('RESUMO DO VALOR')))
    print(f'Preco analisado: {formatacao(valor)}')
    print(f'Dobro do preco: {formatacao(valor * 2)}')
    print(f'Metade do preco: {formatacao(valor / 2)}')
    print(f'{aumento}% de aumento: {formatacao(valor * (1 + (aumento / 100)))}')
    print(f'{reducao}% de reducao: {formatacao(valor * (1 - (reducao / 100)))}')